...
import ChessBoard.TraditionalBoard;
...
public class Controller {
        ...
        MouseListener mouselistener = new MouseListener(){
            public void mouseClicked(MouseEvent e) {
                if(!viewer.pause)
                {
                    ...
                    int newx = (int) ((board.getWidth() * ((JComponent) e).getX()) / viewer.getWidth() ) ;
                    int newy = (int) ((board.getHeight() * (((JComponent) e).getY()-40)) / viewer.getHeight() );
                    boolean movePiece = ((InputEvent) e).isShiftDown();
                    if(movePiece)
                    {
    ...
    {
        if(player ==1)
            return board.getPieces(board.player1, name, index, null );
        else
            return board.getPieces(board.player2, name, index, null);
    }
    public int getW()
